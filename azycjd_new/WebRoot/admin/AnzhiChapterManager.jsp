...
<div class="divInit">
	<div class="divInitOne">
		<div class="divInitTwo">
			章节管理&nbsp;&nbsp;&nbsp;<a href="javaScript:;" onclick="setChapterVip(this,${param.novelId})">手动加V</a>&nbsp;&nbsp;&nbsp;<a href="javaScript:;" onclick="updatecreateNovelChapter(${param.novelId})">生成目录</a>
		</div>
	</div>
					  ...
					  <c:forEach items="${s.ls }" var="k">
			  			<tr>
						  	<td class="${k.readType eq 0?'free_chapter':'vip_chapter' }">第${chaper_num_index}章 ${k.chapterTitle }
								...
								<c:if test="${k.state eq 5 }">
									<font style="color:red;">屏蔽中</font>
								</c:if>
						  	</td>
							...
							<td>
								<div id="actiondiv${k.id }">
									...
									<c:if test="${k.state eq 1 && k.readType eq 1}">
										<a href="javascript:;" onclick="updateChapterReadType(${k.id},this,-1)">取消加V</a>
									</c:if>
									...
									<c:if test="${k.state ne 5}">
										<a href="javascript:;" onclick="updateChapterReadType(${k.id},this,5)">屏蔽</a>
									</c:if>
									<c:if test="${k.state eq 5}">
										<a href="javascript:;" onclick="updateChapterReadType(${k.id},this,1)">取消屏蔽</a>
									</c:if>
									<a id='defdsgleteanzhichapter${k.id}' class='delBtnCss' href='javascript:;' onclick="window.location.href='<%=request.getContextPath()%>/anzhichapter/findAnzhiChapterByIdAdmin.do?anzhichapterId=${k.id}'">[修改]</a>

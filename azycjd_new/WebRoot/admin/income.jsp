...
<div class="divInit">
	...
	<form id="AnzhiWebEditorWorkForm" action="anzhichaptersubscribe/income.do" method="post">
		<div class="searchItemsCssss">
			...
			<div class='searchDivItemText'>
				作品名称：<input class="inputTextCss" type="text" id="novelTitle" name="novelTitle" class="txt" value="${param.novelTitle}"/>
			</div>
			...
			<table width="100%" border="0">
				...
				<c:forEach items="${incomeList}" var="s" varStatus="ind">
					<tr>
						<td>${ind.index+1 }</td>
						<td>${s.writerName }</td>
						<td><a href="javascript:window.open('<%=request.getContextPath()%>/novel/${s.novelId }.jsp')" target="_blank">${s.novelTitle }</a></td>
						<td>${s.zhangjieIncome/2 }</td>
						<td>${s.dashangIncome/2 }</td>

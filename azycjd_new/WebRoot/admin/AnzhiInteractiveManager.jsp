				...
				<c:forEach items="${anzhiinteractiveList.list}" var="s" varStatus="ind">
					<tr>
						...
						<td>
							...
							<c:if test="${s.isjiajing eq 0 }">
								[ <a class="updateRecordCss_a" href="javascript:;" onclick="">加精</a> ]
							</c:if>
							<c:if test="${s.isjiajing eq 1 }">
								[ <a class="updateRecordCss_a" href="javascript:;" onclick="">取消加精</a> ]
							</c:if>
							
							<c:if test="${s.istop eq 0 }">
								[ <a class="updateRecordCss_a" href="javascript:;" onclick="">置顶</a> ]
							</c:if>
							<c:if test="${s.istop eq 1 }">
								[ <a class="updateRecordCss_a" href="javascript:;" onclick="">取消置顶</a> ]
							</c:if>
						</td>
					</tr>
				</c:forEach>

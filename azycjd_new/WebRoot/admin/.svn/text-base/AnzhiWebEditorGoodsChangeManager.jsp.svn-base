		...
		<div class="searchItemsCssss">
			...
			<div class='searchDivItemText' id="stateens">
				<script language="javascript">
					showSelect("state","state","${param.state}",checkTypeData,"stateens","兑换状态：",true,115);
				</script>
			</div>
					...
					<tr>
						...
						<td>
							...
							<c:if test="${loginUsers.memberType eq 2 && s.state eq 0 }">
								<a class='updateBtnCss' href='javascript:;' onclick="adminCheck(${s.id},'${s.anzhiWebEditor.editorName }积分兑换[${s.goodsName }]',17)">[审核]</a>
							</c:if>

...
<form id="AnzhiHandleFormAction" action="<%=request.getContextPath()%>/anzhihandle/addAnzhiHandle.do" method="post">
	<div class="inputItemsCssss">
		...
		<c:if test="${param.handleType eq 4 }">
			<div class='searchDivItemText' id="actionsTypgjgfntadgfghjfdhgsdfhj">
				<script language="javascript">
					showSelect("daiqianyue","daiqianyue","0",daiqianyueTypeData,"actionsTypgjgfntadgfghjfdhgsdfhj","是否待签约：",false,135);
				</script>
			</div>
		</c:if>

		...
		<div class="tableCss">
			<table width="100%" border="0">
				...
				<c:forEach items="${anzhiauthorList.list}" var="s" varStatus="ind">
					<tr>
						<td>${ind.index+1 }</td>
						<td><a href="<%=request.getContextPath()%>/anzhiauthor/findAnzhiAuthorByDetail.do?anzhiauthorId=${s.memberId }" target="_blank">${s.authorWriterName }</a></td>
						<td>${s.memberNick }</td>
						<td title="总共收入：${s.totalMoney }">${s.level }</td>
						<td>${s.authorTel }</td>

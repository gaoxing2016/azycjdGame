var advTypeData=new Array("0,系统公告","1,读者公告","2,作家公告","3,小说详情界面公告","4,诚聘英才","5,用户指南","6,活动专区");
...
var daiqianyueTypeData=new Array("0,不是待签约","1,待签约");
...
var cosumeTypeData=new Array("0,打赏","1,购买");
...
function updateChapterReadType(chapterId,obj,state){
	if(confirm("您确定执行操作吗？")){
		$(obj).slideToggle();
		$.post(path+"/anzhichapter/updateChapterReadType.do",{chapterId:chapterId,state:state},function(res){
			alert("操作成功");
		});
	}
}

function updateChapterRecord(obj){
	if(confirm("您确定执行操作吗？")){
		$.post(path+"/util/updateChapterUpdRecord.do",null,function(res){
			alert(res);
		});
	}
}

function updatecreateNovelChapter(novelId){
	$.post(path+"/anzhichapter/createNovelChapter.do",{novelId:novelId},function(res){
		alert("生成成功！");
	});
}
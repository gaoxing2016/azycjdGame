						...
						<td>
							...
							<c:if test="${k.successInt eq 1 }">
								<c:if test="${k.fuliType eq 0 }">
									<a href="javascript:;" onclick="benefit(${s.id },'${k.vote }',2,this,'${k.jiangliMoney }')">申请</a>
								</c:if>
								<c:if test="${k.fuliType eq 1 }">
									<a href="javascript:;" onclick="benefit(${s.id },'${k.vote }',0,this,'${k.jiangliMoney }')">申请</a>
								</c:if>
								<c:if test="${k.fuliType eq 2 }">
									<a href="javascript:;" onclick="benefit(${s.id },'${k.vote }',1,this,'${k.jiangliMoney }')">申请</a>
								</c:if>
							</c:if>
						</td>
					</tr>
			...
			<tr>
				...
				<td>
					...
					<c:if test="${jingyingjiang.successInt eq 1 }">
						<a href="javascript:;" onclick="benefit(${loginUsers.id },'${jingyingjiang.vote }',9,this,'${k.jiangliMoney }')">申请</a>
					</c:if>
				</td>
			</tr>
			<tr>
				...
				<td>
					...
					<c:if test="${daojufencheng.successInt eq 1 }">
						<a href="javascript:;" onclick="benefit(${loginUsers.id },'${daojufencheng.vote }',10,this,'${k.jiangliMoney }')">申请</a>
					</c:if>
				</td>
			</tr>
		</table>

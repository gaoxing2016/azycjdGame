var paihangbang=new Array("本月月票榜","总点击","总收藏","本月更新榜","本月推荐榜","总订阅榜","本月新书月票榜","本月书友打赏榜","本月新书点击榜","本月新书推荐榜","本月新书收藏榜","本月完结精品榜");
...
function browserRedirect() {
	$.post(path+"/getMobileWeb.jsp",null,function(res){
		if(res=="no"){
...
function memberLogin(obj){
	...
	$.post(path+"/anzhimember/login.do",{
		...
		if(res.indexOf("登录成功")==-1){
			...
			$.post(path+"/anzhimember/passwordEnc.do",{passwordStr:$("#upass").val()},function(res){
				SetCookie("upass",res);
...
function ajaxFormSubmit(id,obj) {
	...
	if(id=="infoupdateform"){
		...
		if(!emailReg.test($("#authorEmail").val())){
			alert("邮箱输入错误！");
			return;
		}
    	...
    	$.ajax({
            ...
            success:function(res){
            	if(id=="AnzhiFenJuanFormActionIndex"){
            		...
            		alert(res);
            		window.location.href=path+"/anzhiinteractive/findAnzhiInteractiveAllByType.do?interactiveType=0";
            	...
            	}else if(id=="AnzhiWebEditorSignForm"){
            		$("#wangbianqiandao").html(res);
            	}else{
            		$("#mainContent").html(res);
            	}
            	$.jBox.closeTip();
...
function del(p){
	if(confirm("您确定删除该条记录吗？")){
		...
		if(p.indexOf("anzhidiscounted/delAnzhiDiscounted.do?anzhidiscountedId")!=-1){
			p+="&&discountedType="+$("#discountedType").val();
		}
		if(p.indexOf("anzhirecommend/delAnzhiRecommend.do?anzhirecommendId")!=-1){
			p+="&&recommendPositions="+$("#recommendPositions").val();
		}
		allLoadPage(path+"/"+p);
	}
}
...
function loadLoginInfoCookie(){
	...
	if(uname==null){
		...
		$.post(path+"/anzhimember/passwordJieMi.do",{passwordStr:upass},function(res){
			$("#uname").val(uname);
			$("#upass").val(res);
		});
	}
...
function benefit(novelId,vote,types,obj,jiangliMoney){
	if(confirm("您确定申请该福利吗？")){
		$(obj).slideToggle();
		$.post(path+"/anzhibenefit/applyAnzhiBenefit.do",{novelId:novelId,benefitType:types,applReason:vote,vote:jiangliMoney},function(res){
			alert("申请成功");
		});
	}
}
...
function loadMobileJsp(){
	$.post(path+"/getMobileWeb.jsp",null,function(res){
		if(res=="no"){
			window.open(path+"/erweima.jsp");
		}else{
			window.location.href=path+"/mobile.jsp";
		}
	});
}
...
function moreJieShao(novelId){
	$("#novel_jieshao").data("oldVlaue",$("#novel_jieshao").html());
	$.post(path+"/anzhinovel/loadNovelVote.do?novelId="+novelId,null,function(res){
		$("#novel_jieshao").html(res+"<span style='cursor: pointer; padding-left: 20px;font-weight: bold; color:#2c95ee;text-decoration: underline;' onclick='shousuojianjie()'>收缩</span>");
	});
}
function shousuojianjie(){
	$("#novel_jieshao").html($("#novel_jieshao").data("oldVlaue"));
}

function delFenJuan(fenJuanId){
	if(confirm("删除该分卷会导致该分卷下的章节无法正常显示，确定删除吗？")){
		$.post(path+"/anzhifenjuan/delAnzhiFenJuan.do?anzhifenjuanId="+fenJuanId,null,function(res){
			if(res=="yes"){
				window.location.reload();
			}else{
				alert(res);

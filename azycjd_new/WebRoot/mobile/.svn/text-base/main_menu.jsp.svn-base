	...
	<div style="clear:both; line-height:60px; padding-left:15px;font-weight:bold;">
	 ...
	 <c:forEach items="${chapterData.list }" var="s" varStatus="ind">
	 	...
	 	<c:forEach items="${s.ls }" var="k">
	 		<div class="user_div_xss" style="background-color:#FFFFFF;">
				<div class="titls_info_td" style="padding-top:5px;cursor:pointer;" onclick="window.location.href='<%=request.getContextPath()%>/mobile/readChapterReal.do?chapterId=${k.id }&&novelId=${param.novelId }'">
		...
		<table width="100%" border="0">
		  <tr style="background-color:#FFFFFF;line-height:45px;font-family:'黑体'; font-weight:bold;">
		   ...
		   <c:forEach items="${markData.list }" var="s" varStatus="ind">
				...
				<td><a href="<%=request.getContextPath()%>/mobile/readChapter.do?chapterId=${s.chapterId }&&novelId=${param.novelId }&&readAddress=${s.readAddress}&&looktype=now">阅读</a></td>
			  </tr>
		   </c:forEach>
		</table>
	</div>

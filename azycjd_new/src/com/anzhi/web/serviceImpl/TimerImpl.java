...
public class TimerImpl extends Thread implements ITimer{
	...
	public void updateChapterState(String path) {
		...
		logger.warn(new SimpleDateFormat().format(new Date())+"---更新了"+ls.getList().size()+"条记录");
		for(int i=0;i<ls.getList().size();i++){
			...
			logger.warn(new SimpleDateFormat().format(new Date())+"---序号："+(i+1)+"更新了【"+ac.getChapterTitle()+"】章节。小说："+ac.getNovelId()+",章节ID："+ac.getId());
		}
	}
	...
	@Override
	public void run() {
		if(this.getPath().indexOf("azycjd_new")==-1){
			while(true){

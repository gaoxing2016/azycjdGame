...
public class AnzhiMemberServiceImpl extends ActionDb  implements IAnzhiMemberService {
	...
	public void everyDayTask(HttpServletRequest request, AnzhiMember t) {
		...
		//每日投票
		int toupiao=this.getIntBySqlDb("select sum(reward_num) from anzhi_novel_reward where reward_type=0 and member_id="+t.getId()+" and add_time between '"+this.getDateType(0)+"' and '"+this.getDateType(1)+"'");
		//每日充值

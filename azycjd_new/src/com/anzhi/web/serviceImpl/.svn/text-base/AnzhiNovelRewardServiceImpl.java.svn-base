...
public class AnzhiNovelRewardServiceImpl extends ActionDb  implements IAnzhiNovelRewardService {
	...
	public String cashDaShang(HttpServletRequest request, AnzhiMember am,
		...
		if(anzhinovelreward.getRewardType().equals(2)){
			...
			int nt=this.getIntBySqlDb("select sum(reward_num) from anzhi_novel_reward where member_id="+am.getId()+" and reward_type=2 and add_time between '"+this.getDateType(4)+"' and '"+this.getDateType(5)+"'");
			if(nt>=yuePiaoNum){
				return "亲，您本月月票已经用完了！";
			}

...
public class AnzhiMessageServiceImpl extends ActionDb  implements IAnzhiMessageService {
	...
	public void loadNoReadMessage(HttpServletRequest request, AnzhiMember am) {
		...
		if(am!=null){
			...
			int friendNum=this.getIntBySqlDb("select count(*) from anzhi_friend where friend_member_id="+am.getId()+" and state=0");
			...
			if(friendNum!=0){
				...
				request.getSession().removeAttribute("friendNum");
			}
			if(zuoJiaNum!=0){

...
public class MobileServiceImpl extends ActionDb implements IMobileService {
	...
	public String mChapterMenu(HttpServletRequest request) {
		...
		AnzhiNovel pojo=new AnzhiNovel();
		if(pl.getList().size()!=0){
			pojo= pl.getList().get(0);
			pojo.setTotalShangNum(this.getIntBySqlDb("select sum(reward_anzhi_money) from anzhi_novel_reward where reward_type>2 and novel_id="+pojo.getId()));
		}

...
public class XmlServiceImpl extends XmlParent implements IXmlService {
	...
	public Element ersansiwu_novelList(HttpServletRequest request) {
		int size=request.getParameter("size")!=null&&request.getParameter("size")!=""?Integer.parseInt(request.getParameter("size")):500;
		int pageNum=this.getPageNum(request);
		String conditon="";
		if(this.getInt(request, "page")!=-1){
			pageNum=this.getInt(request, "page");
		}
		if(this.checkParam(request, "data")){
			if(request.getParameter("data").equals("new")){
		...
		for(int i=0;i<novelList.size();i++){
			ids+=novelList.get(i).getId()+",";
		}
		ids=ids.equals("")?"":" and novel_id in ("+ids.substring(0, ids.length()-1)+")";
		QueryList<AnzhiChapter> chapterLs=anzhichapterDao.findAnzhiChapterAll(5000, 1, " and id in(select max(id) from anzhi_chapter_report where 1=1 "+ids+" group by novel_id)");
		for(int i=0;i<novelList.size();i++){
			...
			item.add(this.addElement("vip", novelFree(novelList.get(i)),false));
			if(novelList.get(i).getNovelType()!=null){
				if( novelList.get(i).getNovelType().length()>2){
					if (novelList.get(i).getNovelType().equals("古代言情") || novelList.get(i).getNovelType().equals("现代言情")) {
						item.add(this.addElement("type", novelList.get(i).getNovelType()));
					} else {
						item.add(this.addElement("type", novelList.get(i).getNovelType().substring(0, 2)));
					}
				}else{
					item.add(this.addElement("type", novelList.get(i).getNovelType()));
				}
			}else{
				item.add(this.addElement("type", ""));
			}
			...
			AnzhiChapter acc=getAnzhiChapter(chapterLs, novelList.get(i).getId());
			int lastId=acc.getId();
			item.add(this.addElement("latest_chapter", acc.getChapterTitle()));
	...
	public AnzhiChapter getAnzhiChapter(QueryList<AnzhiChapter> ls,int novelId){
		AnzhiChapter ac=new AnzhiChapter();
		ac.setId(0);
		for(int i=0;i<ls.getList().size();i++){
			if(ls.getList().get(i).getNovelId().equals(novelId)){
				ac=ls.getList().get(i);
				break;
			}
		}
	...
	public Element changdu_novelByBookId(HttpServletRequest request,String xmlfileName) {
		...
		if(novel!=null){
			...
			bookinfo.add(this.addElement("license", novelFree(novel)));
		}
		return bookinfo;
	}

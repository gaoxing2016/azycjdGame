...
import com.anzhi.web.controller.AddBookSuNing;
...
public class AnzhiThirdRecommandServiceImpl extends ActionDb  implements IAnzhiThirdRecommandService {
	...
	public JSONObject addAnzhiThirdRecommand(AnzhiThirdRecommand anzhithirdrecommand,List list,String path){
		...
		if(list!=null){
			for(int i=0;i<list.size();i++){
				try {
					...
					if((obj[0]!=null&&obj[0].toString().equals(""))||obj[0]==null){
						continue;
					}
		...
		if(con){
			...
			}else if(anzhithirdrecommand.getThirdType().equals(10)){
				for(int i=0;i<list.size();i++){
					try {
						Object[] obj=(Object[])list.get(i);
						JSONObject p=new JSONObject();
						...
						if(ls.getList().size()!=0){
							...
							QueryList<AnzhiChapter> chap=anzhichapterDao.findAnzhiChapterAll(10000, 1, " and novel_id="+Integer.parseInt(obj[1].toString()));
							AddBookSuNing add=new AddBookSuNing(an,chap.getList(),this.getIntBySqlDb("select count(*) from anzhi_chapter_report where novel_id="+an.getId()+" and read_type=0"),this.getStringBySqlDb("select author_info from anzhi_author where member_id="+an.getAuthorId()),path);
						}
					} catch (Exception e) {
						e.printStackTrace();
						resut="小说推送失败，第"+(i+1)+"行数据异常！"+e.getMessage();
					}
				}

...
public class AnzhiAuthorServiceImpl extends ActionDb  implements IAnzhiAuthorService {
	...
	public QueryList<AnzhiAuthor> findAnzhiAuthorAll(int size,int pageNum,String condition){
		QueryList<AnzhiAuthor> list= anzhiauthorDao.findAnzhiAuthorAll(size,pageNum,condition);
		for(int i=0;i<list.getList().size();i++){
			AnzhiAuthor am=list.getList().get(i);
			double dashangAndDingyue=this.getDoubleBySqlDb("select SUM(consume_anzhi_money) from anzhi_consume_novel_v where author_id="+am.getMemberId());
			double quanQinShangjiaWanjie=this.getDoubleBySqlDb("select sum(CAST(vote as int)) from anzhi_benefit_novel where author_id="+am.getMemberId());
			double jingyingDaou=this.getDoubleBySqlDb("select sum(CAST(vote as int)) from anzhi_benefit where (benefit_type=9 or benefit_type=10) and novel_id="+am.getMemberId());
			double total=(dashangAndDingyue/100+quanQinShangjiaWanjie+jingyingDaou);
			am.setTotalMoney(total);
			am.setLevel(this.zuozheLevel(total)+"");
		}
		return list;
	}
	...
	public void fuliShenQin(HttpServletRequest request, AnzhiMember am) {
		...
		for(int i=0;i<ls.getList().size();i++){
			...
			if(yetQuanQin==1){
				...
				for(int s:arr){
					...
					if(n==upMonthDayNum){//说明全勤
						...
						azf.setSuccessInt(1);
						azf.setJiangliMoney(300*(s/3000));
						break;
					}else{
			...
			if(shangjia==1){
				...
				if(num==0){
					String shangjiaDate=this.getStringBySqlDb("select top 1 add_time from anzhi_novel_multifunction where novel_id="+an.getId()+" and (action_type=16 or action_type=17 or action_type=22) order by add_time desc");
				...
				if(an.getState().equals(3)){
					...
					if(!wanjieDate.equals("0")&&!qianyueDate.equals("0")&&!wanjieDate.equals("")&&!qianyueDate.equals("")){
						...
						for(int j=e;j<=b;j++){
							...
							//考勤天数
							int n=this.getIntBySqlDb("select count(*) from (select novel_id,Convert(varchar(100),add_time,23) as riqi,SUM(update_num) as updateNum from anzhi_chapter_update_novel_v where DATEDIFF(month,add_time,GETDATE())="+j+" and novel_id="+an.getId()+" group by novel_id,Convert(varchar(100),add_time,23)) t");
							//当月的天数
						...
						if(con){
							...
							if(an.getTotalCharNum()>=1500000){
								azf2.setJiangliMoney(800);
							}else if(an.getTotalCharNum()>=1000000){
								azf2.setJiangliMoney(500);
							}else if(an.getTotalCharNum()>=800000){
								azf2.setJiangliMoney(300);
							}else if(an.getTotalCharNum()>=500000){
								azf2.setJiangliMoney(150);
							}else if(an.getTotalCharNum()>=350000){
								azf2.setJiangliMoney(100);
							}else{
								azf2.setJiangliMoney(0);
							}
		...
		if(daoju==1){
			...
			if(daojunum>20000){
				...
				azf4.setJiangliMoney((daojunum/100)*0.5);
	...
	public int getUpMonthDayNum(int month){
		...
		Calendar cal = Calendar.getInstance();
		//取得系统当前时间所在月第一天时间对象
		cal.set(Calendar.MONTH, cal.get(Calendar.MONTH)-month+1);
		cal.set(Calendar.DAY_OF_MONTH, 1);
		//日期减一,取得上月最后一天时间对象
		cal.add(Calendar.DAY_OF_MONTH, -1);
		return cal.get(Calendar.DAY_OF_MONTH);

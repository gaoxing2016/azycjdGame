...
public class AnzhiHandleServiceImpl extends ActionDb  implements IAnzhiHandleService {
	...
	public boolean addAnzhiHandle(AnzhiHandle anzhihandle,AnzhiWebEditor aw,HttpServletRequest request){
		...
		if(anzhihandle.getHandleResult().equals(1)){
			...
			if(anzhihandle.getHandleType().equals(2)){
				...
				anzhihandle.setDaiqianyue(this.getInt(request, "daiqianyue"));
	...
	public String adminCheck(HttpServletRequest request, AnzhiMember am) {
		...
		if(types==17){
			...
			if(states==3){
				//如果是拒绝，则退换积分
				int editorId=this.getIntBySqlDb("select editor_id from anzhi_web_editor_goods_change where id="+this.getInt(request, "tableId"));
				int goodsId=this.getIntBySqlDb("select goods_id from anzhi_web_editor_goods_change where id="+this.getInt(request, "tableId"));
				double score=this.getDoubleBySqlDb("select goods_score from anzhi_web_editor_goods where id="+goodsId);
				this.updateTableFieldAppend("anzhi_web_editor", editorId, "editor_score", (int)score);
			}
		}else if(types==14){

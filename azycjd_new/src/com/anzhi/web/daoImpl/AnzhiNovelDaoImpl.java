...
public class AnzhiNovelDaoImpl extends ActionDb implements IAnzhiNovelDao {
	...
	public Pojo updateAnzhiNovel(AnzhiNovel anzhinovel){
		Pojo pojo=new Pojo();
		pojo.setSql("update anzhi_novel set novel_title=?,novel_summary=?,novel_type=?,vote=?,novel_img=?,novel_label=?,authorization_level=?,categories=?,leading_role=?,supporting_role=?,my_editor=?,posted_way=?,en_name=?,is_jinbi=?,is_wangwen=?,is_signed=?,state=? where id=?");
		pojo.setObj(new Object[]{
			anzhinovel.getNovelTitle(),
			anzhinovel.getNovelSummary(),
			anzhinovel.getNovelType(),

...
public class AnzhiAuthorDaoImpl extends ActionDb implements IAnzhiAuthorDao {
	...
	public Pojo updateAnzhiAuthor(AnzhiAuthor anzhiauthor){
		Pojo pojo=new Pojo();
		pojo.setSql("update anzhi_author set member_id=?,author_writer_name=?,author_tel=?,author_qq=?,author_image=?,author_realname=?,author_address=?,author_post=?,author_email=?,author_msn=?,author_credenttype=?,author_credentcode=?,author_info=?,author_state=?,vote=? where id=?");
		pojo.setObj(new Object[]{
			anzhiauthor.getMemberId(),
			anzhiauthor.getAuthorWriterName(),
			anzhiauthor.getAuthorTel(),
	...
	public QueryList<AnzhiAuthor> findAnzhiAuthorAll(int size,int pageNum,String condition){
		return this.excuteQuery(new AnzhiAuthor(),"anzhi_author_view",size,pageNum,condition);
	}

...
public class XmlController extends XmlParent{
	 ...
	 * @return
	 */
	public AnzhiChapter getChapterById(String condition,String huanghang){
		...
		if(ls.getList().size()>0){
			...
			chapterContentReplace(ac,huanghang);
		}
		return ls.getList().size()==0?null:ls.getList().get(0);
	}
	
	public void chapterContentReplace(AnzhiChapter ac,String huanghang){
		String infd=new HtmlStr().getTextFromHtml(ac.getChapterContent());
		String c=infd;
		if(huanghang.equals("</p><p>")){
			c="<p>"+c;
		}
		while(c.indexOf("\r\n")!=-1){
			c=c.replace("\r\n", huanghang);
		}
		while(c.indexOf("<p></p>")!=-1){
			c=c.replace("<p></p>", "");
		}
		while(c.indexOf("<br><br>")!=-1){
			c=c.replace("<br><br>", "");
		}
		while(c.indexOf(" ")!=-1){
			c=c.replace(" ", "");
	...
	public AnzhiChapter getChapterById(String condition){
		return  this.getChapterById(condition,"<br>");
	}

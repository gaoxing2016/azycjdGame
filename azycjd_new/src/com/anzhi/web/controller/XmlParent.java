...
public class XmlParent extends ActionDb{
	...
	//合作方的列表
	String[] opertion=new String[]{"uc","changdu","sanliuling","ersansiwu","aishuke","aiyuedu","dangdang","douding","shuxiang","zhangyue","suning","tadu","wangyi","xunlei","yueduxing","zuidie"};
	 ...
	 * @return
	 */
	public int novelFree(AnzhiNovel an){
		//先看有没有签约
		int n=an.getIsSigned();
		if(n==0){
			//说明没有签约 查询是否网站上架    或   平台上架
			n+=this.getIntBySqlDb("select count(*) from anzhi_novel_multifunction where novel_id="+an.getId()+" and (action_type=17 or action_type=16)");
		}
		return n==0?0:1;
	}
	/**
	...
	public long getShiJianCuo(String t){
		Timestamp ts=null;
		try {
			ts = new Timestamp(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(t).getTime());
		} catch (ParseException e) {
		}
		return getShiJianCuo(ts);
	}
	...
	public String getSubString(String v,int len){
		if(v!=null){
			if(v.length()>len){

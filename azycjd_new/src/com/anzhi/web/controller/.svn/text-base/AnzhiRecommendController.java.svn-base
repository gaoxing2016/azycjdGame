		...
		if(request.getSession().getAttribute("delAnzhiRecommendSession")==null){
			try{
				...
				if(this.checkParam(request, "recommendPositions")){
					response.sendRedirect(request.getContextPath()+"/anzhirecommend/findAnzhiRecommendAll.do?pageNum="+request.getParameter("pageNum")+"&&recommendPositions="+request.getParameter("recommendPositions"));
				}else{

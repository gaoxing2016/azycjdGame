...
public class AnzhiFriendController extends GetCondition {
	...
	public String findMyFriendAll(HttpServletRequest request,HttpServletResponse response){
		...
		int pageNum=this.getPageNum(request);
		//读取我自己的好友
		QueryList<AnzhiFriend> memberlist =anzhifriendService.findAnzhiFriendAll(size, pageNum, " and member_id="+this.getLoginMemberId(request));
		request.setAttribute("memberlist", memberlist);
		//读取别人加我的好友
		QueryList<AnzhiFriend> list =anzhifriendService.findAnzhiFriendAll(size, pageNum, " and friend_member_id="+this.getLoginMemberId(request));
		...
		if(request.getSession().getAttribute("delAnzhiFriendSession")==null){
			try{
				...
				if(this.checkParam(request, "state")){
					anzhifriendService.updateAnzhiFriend(id, this.getInt(request, "state"));
				}else{
	 ...
	 * 查询列表
	 */
	@RequestMapping(value="/anzhifriend/findMyFriendSearch",method=RequestMethod.POST)
	public String findMyFriendSearch(HttpServletRequest request,HttpServletResponse response){
		int size=request.getParameter("size")!=null&&request.getParameter("size")!=""?Integer.parseInt(request.getParameter("size")):10;
		int pageNum=this.getPageNum(request);
		String friendname=request.getParameter("friendname");
		if(friendname!=null&&!friendname.equals("")){
			QueryList<AnzhiMember> friendlist =iAnzhiMemberService.findAnzhiMemberAll(size, pageNum, " and  member_nick like '%"+friendname+"%' ");
			request.setAttribute("friendlist", friendlist);
		}
		return this.getPageName(request, "index/friendlist");
	}
}
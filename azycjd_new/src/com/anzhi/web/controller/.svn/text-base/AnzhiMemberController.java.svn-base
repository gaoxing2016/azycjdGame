...
public class AnzhiMemberController extends GetCondition {
	...
	@RequestMapping(value="/anzhimember/piliangReadMoney")
	public String piliangReadMoney(HttpServletRequest request,HttpServletResponse response){
		return anzhimemberService.piliangReadMoney(request);
	}
	
	@RequestMapping(value="/anzhimember/passwordEnc")
	public void passwordEnc(HttpServletRequest request,HttpServletResponse response) throws IOException{
		response.setCharacterEncoding("utf-8");
		if(this.checkParam(request, "passwordStr")){
			response.getWriter().write(this.getEncString(request.getParameter("passwordStr").trim()));
		}
	}
	
	@RequestMapping(value="/anzhimember/passwordJieMi")
	public void passwordJieMi(HttpServletRequest request,HttpServletResponse response) throws IOException{
		response.setCharacterEncoding("utf-8");
		if(this.checkParam(request, "passwordStr")){
			response.getWriter().write(this.jieMiString(request.getParameter("passwordStr").trim()));
		}
	}
	
	@RequestMapping(value="/anzhimember/addAnzhiMemberMobile",method=RequestMethod.POST)
	public void addAnzhiMemberMobile(HttpServletRequest request,HttpServletResponse response,AnzhiMember anzhimember){
		if(request.getSession().getAttribute("addAnzhiMemberSession")==null){
			try{
				response.setContentType("text/html;charset=utf-8");
				request.getSession().setAttribute("addAnzhiMemberSession","true");
				anzhimember.setAddTime(getTimestamp());
				if(anzhimember.getId()==null||anzhimember.getId()==0){
					if(!this.checkParam(request, "type")){
						response.getWriter().write("参数请求异常");
						return;
					}
					QueryList<AnzhiMember> ls=null;
					if(this.getInt(request, "type")==0){
						ls=anzhimemberService.findAnzhiMemberAll(1, 1, " and (member_tel='"+anzhimember.getMemberTel()+"' or member_no='"+anzhimember.getMemberNo()+"')");
						anzhimember.setMemberEmail("");
					}else if(this.getInt(request, "type")==1){
						ls=anzhimemberService.findAnzhiMemberAll(1, 1, " and (member_email='"+anzhimember.getMemberEmail()+"' or member_no='"+anzhimember.getMemberNo()+"')");
						anzhimember.setMemberTel("");
					}
					if(ls==null||ls.getList().size()!=0){
						response.getWriter().write("您输入的账号或已经"+(this.getInt(request, "type")==0?"手机":"邮箱")+"已经存在");
						return;
					}

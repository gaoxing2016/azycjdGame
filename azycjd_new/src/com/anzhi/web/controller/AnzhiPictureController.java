...
public class AnzhiPictureController extends GetCondition {
	...
	public void loadPicture(HttpServletRequest request,HttpServletResponse response) throws IOException{
		...
		if(request.getSession().getAttribute("isLoadPicData")==null){
			...
			QueryList<AnzhiNews> chengpingyingcai= anzhinewsService.findAnzhiNewsAll(1000, 1, " and news_type=4");
			QueryList<AnzhiNews> yonghuzhinan= anzhinewsService.findAnzhiNewsAll(1000, 1, " and news_type=5");
			QueryList<AnzhiNews> huodongzhuanqu= anzhinewsService.findAnzhiNewsAll(1000, 1, " and news_type=6");
			request.getSession().setAttribute("chengpingyingcai",chengpingyingcai);
			request.getSession().setAttribute("yonghuzhinan",yonghuzhinan);
			request.getSession().setAttribute("huodongzhuanqu",huodongzhuanqu);
